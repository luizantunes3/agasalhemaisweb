...
$pedidos = array();
	...
	$result = mysqli_query($conn,"SELECT * FROM pedidospendentes WHERE ticket=".$_POST['ssearchtxt']);
	...
	while($row = mysqli_fetch_array($result)){
		...
		echo '<tr ticket="' . $row['ticket'] . '">
			<td>' . $row['ticket'] . '</td>
			 ...
			 '<td ><a class="fas fa-recycle" style="Color: yellow;font-size:24px"  onclick=""></a></td>' .
			 '</tr>';
	}
	echo "</table>";
